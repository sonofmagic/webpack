		...
		const processDependencyForResolving = dep => {
			...
			if (resourceIdent !== undefined && resourceIdent !== null) {
				...
				if (list === undefined) {
					...
					if (
						"resourceResolveData" in module &&
						/** @type {NormalModule} */ (module).resourceResolveData.context &&
						/** @type {NormalModule} */ (module).resourceResolveData.context
							.fromLoaderContext
					) {
						entry.contextInfo = { fromLoaderContext: true };
					}
					sortedDependencies.push(entry);
				}
				list.push(dep);
